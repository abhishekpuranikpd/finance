...
export default function CreateSchemeForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("/api/create-scheme", {
      method: "POST",
    ...
    if (response.ok) {
      toast(`{
        ...
        description: ${data.name} successfully created.,
      }`)
      setFormData({
        name: "",
        description: "",
              ...
              <div className="space-y-3">
                ...
                <div className="relative group">
                  ...
                  <Input
                    ...
                    disabled={loading}
                  />
                  {formData.monthlyAmount && <CheckCircle2 className="absolute right-4 top-4 h-5 w-5 text-green-500" />}
                </div>
              </div>

              {/* Total Months */}
              <div className="space-y-3">
                <Label htmlFor="totalMonths" className="text-gray-800 font-semibold flex items-center gap-2 text-base">
                  <div className="p-1.5 bg-emerald-100 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="text-center p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-emerald-100 shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105">
                      ...
                      <p className="font-bold text-blue-700 text-lg">
                        {calculateYearsMonths().years > 0 && `${calculateYearsMonths().years}y `}
                        {`calculateYearsMonths().months > 0 && ${calculateYearsMonths().months}m`}
                        {calculateYearsMonths().years === 0 && calculateYearsMonths().months === 0 && "0m"}
                      </p>
                    </div>
